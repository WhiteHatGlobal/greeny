...
def get_columns(filters: StockBalanceFilter):
	...
	columns.extend(
		[
			...
			{
				"label": _("Stock UOM 2"),
				"fieldname": "default_unit_of_measure_2",
				"fieldtype": "Link",
				"options": "UOM",
				"width": 90,
			},
			{
				"label": _("Balance Qty 1"),
				"fieldname": "bal_qty",
				"fieldtype": "Float",
				"width": 100,
				"convertible": "qty",
...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
	...
	for d in sle:
		...
		# frappe.errprint("2222222222222222222222")
		# frappe.errprint(group_by_key)
		...
		if d.posting_date < from_date or (
			...
			if flt(qty_diff, float_precision) >= 0:
				...
				qty_dict.in_qty_1 += flt(d.qty_2_change)
				qty_dict.in_val += value_diff
				frappe.errprint(qty_dict.in_qty)
			else:
				qty_dict.out_qty += abs(qty_diff)
				qty_dict.out_qty_2 +=flt(d.qty_2_change) - flt(qty_dict.bal_qty_2)
				qty_dict.out_val += abs(value_diff)
				

		qty_dict.val_rate = d.valuation_rate
		qty_dict.bal_qty += qty_diff
		qty_dict.bal_qty_2 += d.qty_2_after_transaction
		qty_dict.bal_val += value_diff
		qty_dict.stock_value_difference += d.stock_value
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockBalanceFilter):
	...
	query = (
		...
		.select(
			...
			item_table.default_unit_of_measure_2,
